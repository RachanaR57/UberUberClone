...
public class PassengerActivity extends FragmentActivity implements OnMapReadyCallback, View.OnClickListener {
    ...
    private Button buttonRequestCar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonRequestCar = findViewById(R.id.buttonRequestCar);
        buttonRequestCar.setOnClickListener(this);
        ...
        carRequestQuery.findInBackground(new FindCallback<ParseObject>() {
            ...
            public void done(List<ParseObject> objects, ParseException e) {
                if (objects.size() > 0 && e == null) {
                    ...
                    buttonRequestCar.setText("Cancel Request");
                }
            }
        });

        findViewById(R.id.buttonLogoutPassenger).setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    public void onClick(View v) {
        if (isUberCancelled) {
            if (ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
                 locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 0, 0, locationListener);
                 Location currentLocPassenger = locationManager.getLastKnownLocation(LocationManager.GPS_PROVIDER);
                if (currentLocPassenger != null) {
                    ParseObject requestCar = new ParseObject("RequestCar");
                    requestCar.put("username", ParseUser.getCurrentUser().getUsername());
                    ParseGeoPoint userLocation = new ParseGeoPoint(currentLocPassenger.getLatitude(), currentLocPassenger.getLongitude());
                    requestCar.put("passengerLocation", userLocation);
                    requestCar.saveInBackground(new SaveCallback() {
                        @Override
                        public void done(ParseException e) {
                            if (e == null) {
                                Toast.makeText(PassengerActivity.this, "Request for car sent", Toast.LENGTH_SHORT).show();
                                buttonRequestCar.setText("Cancel Request");
                                isUberCancelled = false;
            ...
            carRequestQuery.findInBackground(new FindCallback<ParseObject>() {
                ...
                public void done(List<ParseObject> requestList, ParseException e) {
                    if (requestList.size() > 0 && e == null) {
                        ...
                        buttonRequestCar.setText("Request Car");
